...
def initial_population(data, matrix, free, filled, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    for index, class_ in classes.items():
        ind = 0
        # ind = random.randrange(len(free) - int(class_.duration))
        while True:
            start_field = free[ind]

            # check if class won't start one day and end on the next
            start_time = start_field[0]
            end_time = start_time + int(class_.duration) - 1
            ...
            for i in range(1, int(class_.duration)):
            ...
            if start_field[1] not in class_.classrooms:
                ind += 1
                continue

            if found:
                for group_index in class_.groups:
                    # add order of the subjects for group
                    insert_order(subjects_order, class_.subject, group_index, class_.type, start_time)
                    # add times of the class for group
                    for i in range(int(class_.duration)):
                ...
                for i in range(int(class_.duration)):
                    ...
                    teachers_empty_space[class_.teacher].append(i + start_time)
                break
