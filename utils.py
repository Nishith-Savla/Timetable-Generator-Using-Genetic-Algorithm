...
def load_data(file_path, teachers_empty_space, groups_empty_space, subjects_order):
    ...
    for type_ in data['Classrooms']:
        for name in data['Classrooms'][type_]:
            new = Classroom(name, type_)
...
def set_up(num_of_columns):
    ...
    :return: matrix, free
    """
    w, h = num_of_columns, 40                                          # 5 (workdays) * 8 (work hours) = 40
...
def write_solution_to_file(matrix, data, filled, filepath, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    """
    f = open('solution_files/sol_' + os.path.splitext(filepath)[0] + '.txt', 'w')
    ...
    for class_index, times in filled.items():
        ...
        f.write('\nClassroom: {:2s}\nTime: {}'.format(room[:room.rfind('-')], days[times[0][0] // 8]))
        for time in times:
