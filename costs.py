def subjects_order_cost(subjects_order):
    """
    Calculates percentage of soft constraints - order of subjects (L, T, P).
    ...
    print("Subject Order Cost:", cost, total, 100 * (total - cost) / total)
...
def empty_space_groups_cost(groups_empty_space):
    ...
    for group_index, times in groups_empty_space.items():
        ...
        for i in range(1, len(times) - 1):
            ...
            b = times[i]
            diff = b - a
            # classes are in the same day if their time div 8 is the same
    ...
    print("Empty Space Groups Cost:", cost, max_empty, cost / len(groups_empty_space))
...
def empty_space_teachers_cost(teachers_empty_space):
    ...
    print("Empty Space Teachers Cost:", cost, max_empty, cost / len(teachers_empty_space))
...
def hard_constraints_cost(matrix, data):
    """
    Calculates total cost of hard constraints:
        1. In every classroom is at most one class at a time,
        2. Every class is in one of his possible classrooms,
        3. Every teacher holds at most one class at a time and
        4. Every group attends at most one class at a time.
...
def check_hard_constraints(matrix, data):
    ...
    :return: number of overlaps with classes, classrooms, teachers and groups. """
    overlaps = 0
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            field = matrix[i][j]                                    # for every field in matrix
